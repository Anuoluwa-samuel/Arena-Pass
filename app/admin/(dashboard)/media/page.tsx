...
export default async function MediaPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const result = await listMedia({ folder: sp.folder, q: sp.q, page: Number(sp.page ?? 1), pageSize: 40 })
  return (
    <div className="space-y-6">
      ...
      <MediaLibrary items={result.items.map((m) => ({ id: m.id, url: m.url, originalName: m.originalName, altText: m.altText, folder: m.folder, mimeType: m.mimeType, sizeBytes: m.sizeBytes, width: m.width, height: m.height, createdAt: m.createdAt.toISOString() }))} canManage={user.permissions.includes("media.manage")} />
