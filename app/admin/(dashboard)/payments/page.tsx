...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const result = await listPayments({ status: sp.status, q: sp.q, page: Number(sp.page ?? 1), pageSize: 25 })
    ...
    <div className="space-y-6">
      ...
      <Suspense>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <FilterTabs options={[{ value: "all", label: "All" }, { value: "PAID", label: "Paid" }, { value: "PENDING", label: "Pending" }, { value: "FAILED", label: "Failed" }, { value: "REFUNDED", label: "Refunded" }]} />
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "status", header: "Status", cell: (r) => <div className="flex flex-wrap gap-1"><PaymentStatusBadge status={r.payment.status} />{r.ticket?.status && r.ticket.status !== "CONFIRMED" && <TicketStatusBadge status={r.ticket.status} />}</div> },
