...
export default async function NotificationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const result = await listNotifications({ status: sp.status, channel: sp.channel, page: Number(sp.page ?? 1), pageSize: 30 })
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "actions", header: "", className: "text-right", cell: (n) => (n.status === "FAILED" && user.permissions.includes("notifications.manage") ? <RetryNotificationButton id={n.id} /> : null) },
        ]}
      />
