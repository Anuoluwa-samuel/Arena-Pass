...
export default async function ValidatePage() {
  const user = await requirePermission("tickets.validate")
  const now = new Date()
  const { items } = await listSessions({ arenaId: user.arenaId ?? undefined, from: new Date(now.getTime() - 6 * 3_600_000), to: new Date(now.getTime() + 48 * 3_600_000), pageSize: 20 })
