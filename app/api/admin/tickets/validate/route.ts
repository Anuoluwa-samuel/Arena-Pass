...
export const POST = adminRoute("tickets.validate", async (req, _ctx, user) => {
  ...
  const outcome = await validateTicket(body.code, { mode: body.mode, expectedSessionId: body.sessionId, actor: actorFrom(user, req) })
