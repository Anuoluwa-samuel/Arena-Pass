...
export const POST = adminRoute("tickets.manage", async (req, { params }, user) => {
  const { id } = await params
  const { reason } = await parseJson(req, z.object({ reason: z.string().trim().min(3).max(500) }))
  return ok(await cancelTicket(id, reason, { actor: actorFrom(user, req) }), { message: "Ticket cancelled" })
})
