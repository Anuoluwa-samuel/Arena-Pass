...
export const POST = adminRoute("media.manage", async (req, _ctx, user) => {
  ...
  const row = await uploadMedia({ buffer, originalName: file.name, mimeType: file.type }, { arenaId: await resolveArenaId(user), folder, altText, actor: actorFrom(user, req) })
