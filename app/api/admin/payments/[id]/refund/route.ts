...
export const POST = adminRoute("tickets.refund", async (req, { params }, user) => {
  const { id } = await params
  const { reason } = await parseJson(req, z.object({ reason: z.string().trim().min(3).max(500) }))
  return ok(await refundPayment(id, reason, { actor: actorFrom(user, req) }), { message: "Refund issued" })
})
