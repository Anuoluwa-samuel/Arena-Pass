...
export default async function AccountHomePage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      ...
      {missing.length > 0 && (
        <Reveal trigger="mount" delay={0.3} className="mt-8">
      ...
      <div className="mt-10 grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        <Reveal trigger="mount" delay={0.35}>
          <Spotlight className="glass h-full rounded-3xl p-6 sm:p-8">
            <SectionLabel>Next match</SectionLabel>
            {next ? (
              <div className="mt-5 grid gap-6 sm:grid-cols-[1.2fr_1fr] sm:items-end">
                <div>
                  <p className="font-display text-4xl font-semibold uppercase leading-none">{next.session.title}</p>
              ...
              <div className="mt-5 flex flex-col items-start gap-5">
                <p className="font-display text-4xl font-semibold uppercase leading-none text-muted-foreground">No games booked</p>
                <ArrowButton asChild variant="primary" size="sm">
        ...
        <StaggerGroup trigger="mount" delayChildren={0.4} className="grid grid-cols-2 gap-4 lg:grid-cols-1">
          {[
            ...
            <StaggerItem key={stat.label} className="h-full">
              <Spotlight className="glass flex h-full flex-col justify-between rounded-2xl p-5">
                <div className="flex items-center justify-between">
                  <p className="label-mono text-muted-foreground">{stat.label}</p>
                  <stat.icon className="size-4 text-primary" />
                </div>
                <p className="mt-6 font-display text-6xl font-semibold tabular-nums"><CountUp value={stat.value} /></p>
              </Spotlight>
            </StaggerItem>
          ))}
        </StaggerGroup>
      ...
      <section className="mt-16">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-semibold uppercase">Open sessions</h2>
          </div>
