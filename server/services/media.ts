...
export async function uploadMedia(
  ...
  opts: { arenaId: string | null; folder?: string; altText?: string; actor: AuditActor }
) {
...
export async function listMedia(opts: { folder?: string; q?: string; page?: number; pageSize?: number } = {}) {
  const database = await db()
  const page = opts.page ?? 1
  const pageSize = opts.pageSize ?? 40
  const where: SQL[] = [isNull(schema.media.deletedAt)]
  ...
  const folders = await database.selectDistinct({ folder: schema.media.folder }).from(schema.media).where(isNull(schema.media.deletedAt))
...
export async function getMedia(id: string) {
  const database = await db()
  const row = await database.query.media.findFirst({ where: and(eq(schema.media.id, id), isNull(schema.media.deletedAt)) })
  if (!row) throw notFound("Media")
  return row
}

export async function updateMedia(id: string, patch: { altText?: string; folder?: string }, ctx: { actor: AuditActor }) {
  const database = await db()
  const [row] = await database.update(schema.media).set({ ...patch }).where(eq(schema.media.id, id)).returning()
  if (!row) throw notFound("Media")
...
export async function deleteMedia(id: string, ctx: { actor: AuditActor }) {
  const database = await db()
  const row = await getMedia(id)
  await database.update(schema.media).set({ deletedAt: new Date() }).where(eq(schema.media.id, id))
