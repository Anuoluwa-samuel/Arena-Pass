...
import { customer, makeOpenSession } from "../helpers/fixtures"
...
describe("full status and held slots agree", () => {
  it("a live hold on the last slot reads Sold Out, not Open", async () => {
    ...
    await createBooking({ sessionId: session.id, customer: customer(900), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })

    const read = await getSessionById(session.id)
  ...
  it("an expired hold is released on read, so the session is Open again and bookable", async () => {
    const session = await makeOpenSession(ctx.db, { teamsCount: 1, playersPerTeam: 1 })
    const first = await createBooking({ sessionId: session.id, customer: customer(910), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    ...
    const second = await createBooking({ sessionId: session.id, customer: customer(911), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
  ...
  it("does not release a hold that is still live", async () => {
    ...
    const live = await createBooking({ sessionId: session.id, customer: customer(920), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    const read = await getSessionById(session.id)
  ...
  it("the list sweep frees expired holds across sessions", async () => {
    const session = await makeOpenSession(ctx.db, { teamsCount: 1, playersPerTeam: 1 })
    const hold = await createBooking({ sessionId: session.id, customer: customer(930), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
