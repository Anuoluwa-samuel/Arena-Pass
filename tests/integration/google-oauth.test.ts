...
describe("resolveGoogleCustomer", () => {
  it("creates a passwordless customer on first sign-in, then reuses it", async () => {
    const first = await resolveGoogleCustomer(claims("sub-new", "new@example.com"))
    expect(first.created).toBe(true)
    expect(first.customer.passwordHash).toBeNull()
    expect(first.customer.googleSub).toBe("sub-new")

    const again = await resolveGoogleCustomer(claims("sub-new", "new@example.com"))
  ...
  it("finds a linked account by Google id even after the email changed", async () => {
    const { customer } = await resolveGoogleCustomer(claims("sub-moved", "before@example.com"))
    const moved = await resolveGoogleCustomer(claims("sub-moved", "after@example.com"))
    expect(moved.customer.id).toBe(customer.id)
  })

  it("links a guest checkout customer with the same email", async () => {
    const [guest] = await ctx.db.insert(schema.customers).values({ name: "Guest", email: "guest-link@example.com" }).returning()
    const result = await resolveGoogleCustomer(claims("sub-guest", "guest-link@example.com"))
  ...
  it("on first link to a password account, clears the password and revokes its sessions", async () => {
    const [existing] = await ctx.db
      ...
      .values({ name: "Has Password", email: "haspw@example.com", passwordHash: await hashPassword("set-by-someone-1") })
      .returning()
    await createAuthSession("customer", existing.id, {})

    const result = await resolveGoogleCustomer(claims("sub-haspw", "HasPw@example.com"))
    ...
    const later = await resolveGoogleCustomer(claims("sub-haspw", "haspw@example.com"))
  ...
  it("refuses an email already linked to a different Google account", async () => {
    await resolveGoogleCustomer(claims("sub-owner", "taken@example.com"))
    await expect(resolveGoogleCustomer(claims("sub-intruder", "taken@example.com"))).rejects.toMatchObject({ code: "OAUTH_FAILED" })
  })

  it("refuses a deactivated account", async () => {
    await ctx.db.insert(schema.customers).values({ name: "Off", email: "off@example.com", isActive: false })
    await expect(resolveGoogleCustomer(claims("sub-off", "off@example.com"))).rejects.toMatchObject({ code: "ACCOUNT_DISABLED" })
  })
})
