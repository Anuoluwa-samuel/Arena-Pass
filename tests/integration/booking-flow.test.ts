...
async function bookAndPay(sessionId: string, i: number) {
  const { booking } = await createBooking(
    { sessionId, customer: customer(i), idempotencyKey: randomUUID() },
    { actor: { type: "customer" } }
  )
...
describe("session capacity: 8 teams × 4 players = 32", () => {
  ...
  it("never allocates more than 32 slots when 50 customers book simultaneously", async () => {
    const session = await makeOpenSession(ctx.db)
    const attempts = Array.from({ length: 50 }, (_, i) =>
      createBooking({ sessionId: session.id, customer: customer(100 + i), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    )
  ...
  it("balances allocation across teams (round-robin)", async () => {
    ...
    const first = []
    for (let i = 0; i < 8; i++) {
      const r = await createBooking({ sessionId: session.id, customer: customer(200 + i), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
      first.push(r.slot)
    }
    expect(first.map((s) => s.teamNumber)).toEqual([1, 2, 3, 4, 5, 6, 7, 8])
    expect(first.every((s) => s.slotNumber === 1)).toBe(true)
    const preferred = await createBooking({ sessionId: session.id, customer: customer(299), idempotencyKey: randomUUID(), preferredTeamNumber: 3 }, { actor: { type: "customer" } })
  ...
  it("respects a custom team structure (5 teams × 3 players = 15)", async () => {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 20 }, (_, i) => createBooking({ sessionId: session.id, customer: customer(300 + i), idempotencyKey: randomUUID() }, { actor: { type: "customer" } }))
    )
...
describe("booking idempotency and hold expiry", () => {
  it("returns the same booking for a repeated idempotency key", async () => {
    ...
    const a = await createBooking({ sessionId: session.id, customer: customer(400), idempotencyKey: key }, { actor: { type: "customer" } })
    const b = await createBooking({ sessionId: session.id, customer: customer(400), idempotencyKey: key }, { actor: { type: "customer" } })
  ...
  it("releases expired holds so the slot can be re-booked", async () => {
    ...
    const first = await createBooking({ sessionId: session.id, customer: customer(500), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    await expect(
      createBooking({ sessionId: session.id, customer: customer(501), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    ...
    const second = await createBooking({ sessionId: session.id, customer: customer(501), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
  ...
  it("rejects bookings outside the window and for drafts", async () => {
    ...
    await expect(createBooking({ sessionId: closed.id, customer: customer(600), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })).rejects.toMatchObject({ code: "BOOKING_CLOSED" })
    ...
    await expect(createBooking({ sessionId: notYet.id, customer: customer(601), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })).rejects.toMatchObject({ code: "BOOKING_NOT_OPEN" })
    const draft = await makeOpenSession(ctx.db, { publish: false })
    await expect(createBooking({ sessionId: draft.id, customer: customer(602), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })).rejects.toMatchObject({ code: "SESSION_NOT_BOOKABLE" })
  })
})
...
describe("payment verification issues exactly one ticket", () => {
  it("confirms the booking, issues a ticket once, and marks the session FULL at capacity", async () => {
    ...
    const again = await verifyPayment(payment.reference)
  ...
  it("does not confirm a failed or pending payment", async () => {
    const session = await makeOpenSession(ctx.db)
    const { booking } = await createBooking({ sessionId: session.id, customer: customer(710), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    const { payment } = await initializePayment(booking.id)
    expect((await verifyPayment(payment.reference)).status).toBe("PENDING")
    await setMockOutcome(payment.reference, "failed")
    expect((await verifyPayment(payment.reference)).status).toBe("FAILED")
  ...
  it("reuses the pending payment on repeated initialize calls", async () => {
    const session = await makeOpenSession(ctx.db)
    const { booking } = await createBooking({ sessionId: session.id, customer: customer(720), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
    const a = await initializePayment(booking.id)
    const b = await initializePayment(booking.id)
...
describe("ticket validation", () => {
  it("admits a valid ticket once and rejects the second scan", async () => {
    ...
    const payload = buildQrPayload(ticket.qrToken)

    const check = await validateTicket(payload, { mode: "check", actor })
    expect(check.result).toBe("VALID")

    const admit = await validateTicket(payload, { mode: "admit", actor })
    expect(admit.result).toBe("VALID")
    expect(admit.result === "VALID" && admit.ticket.ticket.status).toBe("USED")

    const again = await validateTicket(payload, { mode: "admit", actor })
    expect(again.result).toBe("ALREADY_USED")

    const byNumber = await validateTicket(ticket.ticketNumber, { mode: "check", actor })
  ...
  it("rejects forged payloads and wrong-session scans", async () => {
    ...
    const admin = await getAdminUser(ctx.db)
    const actor = { ...testActor, id: admin.id }
    expect((await validateTicket(`AP1.${ticket.qrToken}.forgedsignature000000`, { mode: "admit", actor })).result).toBe("INVALID")
    expect((await validateTicket("garbage", { mode: "admit", actor })).result).toBe("INVALID")
    expect((await validateTicket(buildQrPayload(ticket.qrToken), { mode: "admit", actor, expectedSessionId: other.id })).result).toBe("WRONG_SESSION")
...
describe("refunds", () => {
  it("refunds a paid ticket, frees the slot and records a ledger entry", async () => {
    ...
    expect((await validateTicket(buildQrPayload(ticket.qrToken), { mode: "admit", actor: { ...testActor, id: admin2.id } })).result).toBe("NOT_VALID")
    // Slot is bookable again.
    const rebook = await createBooking({ sessionId: session.id, customer: customer(901), idempotencyKey: randomUUID() }, { actor: { type: "customer" } })
